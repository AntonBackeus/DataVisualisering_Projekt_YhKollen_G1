...
def swe_map():
    ...
    for region in df_regions["Län"]:
        region_name = get_close_matches(region, regions_codes.keys(), n=1)
        if region_name != []:
            code = regions_codes[region_name[0]]
    ...
    return region_codes_map, properties
...
#    fig.update_layout(
#        mapbox=dict(center=dict(lat=62.6952, lon=13.9149))
#        title=dict(
#            text=f"""
#                    <b>Antal beviljade</b>
